...
The `<DataTableFilter />` component now supports localization!

You can use the optional `locale` prop to control the language of the filter UI.
...
If you don't provide a `locale` prop, it will default to `"en"`.
